...
To use these byte-strings, you need to convert them to something like an open file.
...
## Resizing images
`sg.Image` and `sg.ImageButton` take `width` and `height` as options.

If you define neither, the image doesn't get resized.

Defining either `width`, or `height` (but not both), will resize the image, while keeping its aspect-ratio intact:
```py
layout:list[list[sg.BaseElement]] = [
    [
        sg.Image(
...
## .update
You can change `image`, `width` and `height` using `.update(...)`.

If you change the image that way, `width` and `height` will be reapplied.
...
It has the same options as normal `sg.Button`, but without anything related to its text.

## Normal image as button
...
`icon` can be modified using `.update(...)` and can take any image-format `sg.Image` can.
